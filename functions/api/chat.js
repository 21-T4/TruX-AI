...
async function getImageCount(userIdentifier, env) {
  if (!env.IMAGE_LIMIT_KV) throw new Error('IMAGE_LIMIT_KV binding is missing.');

  const key = `img_limit_${userIdentifier}_${getCurrentMonthKey()}`;
  const value = await env.IMAGE_LIMIT_KV.get(key);
...
async function recordSuccessfulImage(userIdentifier, previousCount, env) {
  const key = `img_limit_${userIdentifier}_${getCurrentMonthKey()}`;
  await env.IMAGE_LIMIT_KV.put(key, String(previousCount + 1));
}
  ...
  try {
    ...
    if (forceImageGen || (isImageGenerationRequest(message) && !image)) {
      ...
      try {
        ...
        await recordSuccessfulImage(userIdentifier, previousCount, env);
    ...
    currentParts.push({ text: message || 'Analyze the provided image.' });
    ...
    return createSseResponse(async ({ send }) => {
      ...
      const firstResult = await streamVertexGemini({
        ...
        accessToken,
        projectId,
        generationConfig: advancedThinking
          ? {
              thinkingConfig: {
